...
def export_csv_PUND_origin_friendly(Dataframe_list, metadata_dict_PUND, output_path, base_name):

    if not metadata_dict_PUND.get("PUND_present", False):
        return
    
    output_csv_data_path = os.path.join(output_path, "CSV_data")
    os.makedirs(output_csv_data_path, exist_ok=True)
    ...
    base_cols = ["Time [s]", "V [V]", "I [A]", "P [uC/cm2]"]
    df_blocks = []
    ...
    for df in Dataframe_list:
        ...
        # Cause in the dataframe the columns are named with suffixes like "Time [s].1", "Time [s].2" for the XPUND pulses
        # We want to merge all of them together respectively
        df = df.copy()
        cycle_value = df["Cycle"].iloc[0]


        DATAFRAME_TEMP ={}  #Dictionary empty

        for col_name in base_cols:
            COL_CONCAT = []

            for col in df.columns:
                if col == col_name or col.startswith(col_name + "."):
                    COL_CONCAT.append(df[col])
            
            COL_CONCAT = np.concatenate(COL_CONCAT)
            DATAFRAME_TEMP[col_name] = COL_CONCAT

        DATAFRAME_TEMP["Cycle"] = np.full(len(COL_CONCAT), cycle_value)
        DATAFRAME_TEMP = pd.DataFrame(DATAFRAME_TEMP)
        DATAFRAME_TEMP = DATAFRAME_TEMP[["Cycle"] + base_cols]  # To place "Cycle" column on the left of the other columns


        DATAFRAME_FINAL.append(DATAFRAME_TEMP)
    ...
    with open(csv_filename, 'w', newline='') as f:
        f.write("".join(values_list) + "\n\n")  # metadata and two empty rows
    DATAFRAME_FINAL.to_csv(csv_filename, index=False, sep='\t', mode='a') 